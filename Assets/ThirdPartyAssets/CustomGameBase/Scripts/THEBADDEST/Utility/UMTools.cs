...
namespace THEBADDEST
{
	...
	public static class UMTools
	{
		...
		public static T FindChildByRecursion<T>(this Transform aParent, string aName)
		{
			T result = default(T);
			var transform = aParent.FindChildByRecursion(aName);
			if (transform != null)
			{
		...
		public static List<T> FindChildrenDeepWithTag<T>(this Transform parent, string tag, bool includeInactive = false) where T : Component
		{
			var componentsInChildren = parent.GetComponentsInChildren<T>(includeInactive);
			List<T> children = new List<T>();
			foreach (T child in componentsInChildren)
			{
		...
		public static List<T> FindChildrenWithTag<T>(this Transform parent, string tag)
		{
			List<T> children = new List<T>();
		...
		public static List<Transform> FindChildrenWithTag(this Transform parent, string tag)
		{
			...
			foreach (Transform child in parent.transform)
			{
				if (child.CompareTag(tag)) children.Add(child);
			}

			return children;
		}

		public static List<T> GetChildrenWithType<T>(this Transform parent)
		{
			List<T> children = new List<T>();
		...
		public static T GetComponentFirst<T>(this Component component) where T : Component
		{
		...
		/// <summary>
		///  usage  int random = GetRandomValue(new RandomSelection(0, 5, .5f),new RandomSelection(6, 8, .3f),new RandomSelection(9, 10, .2f)new RandomSelection(9, 10, .2f));
		/// </summary>
